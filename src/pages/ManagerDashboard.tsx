            ...
            <div className="flex items-start justify-between gap-4 flex-wrap">
              ...
              {b.status === 'pending' && (
                <div className="flex flex-col gap-2 min-w-[160px]">
                  ...
                  <button onClick={() => action(b.id, 'approve')} className="w-full py-2 bg-green-600 hover:bg-green-700 text-white text-sm font-semibold rounded-lg transition-colors">
                    ✓ قبول
                  </button>
                  <button onClick={() => action(b.id, 'reject')} className="w-full py-2 border border-red-300 text-red-600 hover:bg-red-50 text-sm font-semibold rounded-lg transition-colors">
                    ✕ رفض
                  </button>
                </div>
              )}
...
function StatusBadge({ status }: { status: string }) {
  const cls: Record<string, string> = {
    ...
    rejected: 'bg-red-100 text-red-700', on_process: 'bg-blue-100 text-blue-700',

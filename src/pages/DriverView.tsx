        ...
        {tasks.map(task => (
          <div key={task.id} className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
            ...
            {task.status === 'approved' && (
              <button onClick={() => updateStatus(task.id, 'on_process')} className="w-full py-2.5 bg-brand-600 hover:bg-brand-700 text-white font-semibold rounded-xl transition-colors text-sm">
                قبول المهمة — بدء التنفيذ
              </button>
            )}
            {task.status === 'on_process' && (
              <button onClick={() => updateStatus(task.id, 'completed')} className="w-full py-2.5 bg-green-600 hover:bg-green-700 text-white font-semibold rounded-xl transition-colors text-sm">
                ✓ تم الانتهاء
              </button>
            )}
          </div>
